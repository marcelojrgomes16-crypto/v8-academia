...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!p1u) {
      p1u = await prisma.usuario.create({ data: { email: 'carlos@v8academia.com', cpf: '11111111111', nome: 'Carlos Silva', senhaHash: pw, role: 'PROFESSOR', status: 'ATIVO', genero: 'MASCULINO', professor: { create: { cref: 'CREF-12345', especialidades: ['Musculacao', 'CrossFit'], bio: 'Personal trainer experiente.' } } } })
      logs.push('Professor Carlos criado')
    }

    let p2u = await prisma.usuario.findFirst({ where: { email: 'ana@v8academia.com' } })
    if (!p2u) {
      p2u = await prisma.usuario.create({ data: { email: 'ana@v8academia.com', cpf: '22222222222', nome: 'Ana Souza', senhaHash: pw, role: 'PROFESSOR', status: 'ATIVO', genero: 'FEMININO', professor: { create: { cref: 'CREF-67890', especialidades: ['Pilates', 'Yoga'], bio: 'Instrutora certificada.' } } } })
    ...
    if (!joaoExists && p1u && pl1) {
      ...
      const joao = await prisma.usuario.create({ data: {
        ...
        aluno: { create: { matricula: 'V8MASC01', dataMatricula: new Date(), dataVencimento: new Date(Date.now() + 30*86400000), planoId: planoPremium?.id || pl1.id, professorId: p1u.id, objetivo: 'Ganhar massa muscular e forca' } }
      }, include: { aluno: true } })

      const t1 = await prisma.treino.create({ data: { nome: 'Treino A - Peito e Triceps', descricao: 'Treino focado em peito e triceps com cargas progressivas', alunoId: joao.id, professorId: p1u.id, status: 'ATIVO', dataInicio: new Date(), diasSemana: [1, 3, 5] } })
      const exercises1 = [
    ...
    if (!mariaExists && p2u && pl1) {
      ...
      const maria = await prisma.usuario.create({ data: {
        ...
        aluno: { create: { matricula: 'V8FEM01', dataMatricula: new Date(), dataVencimento: new Date(Date.now() + 30*86400000), planoId: planoPremium?.id || pl1.id, professorId: p2u.id, objetivo: 'Tonificar o corpo e melhorar resistencia' } }
      }, include: { aluno: true } })

      const t3 = await prisma.treino.create({ data: { nome: 'Treino A - Tonificacao', descricao: 'Treino focado em tonificacao e resistencia muscular', alunoId: maria.id, professorId: p2u.id, status: 'ATIVO', dataInicio: new Date(), diasSemana: [1, 3, 5] } })
      const ex3 = [
