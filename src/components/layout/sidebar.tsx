...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 z-40 h-screen w-64 border-r border-red-900/20 bg-[#0a0a0a] flex flex-col transition-transform duration-300',
          'lg:translate-x-0',
          open ? 'translate-x-0' : '-translate-x-full'
        )}
      >
        <div className="flex h-16 items-center justify-center border-b border-red-900/20 px-4">
          <Link href={isAdmin ? '/admin' : '/dashboard'} className="flex items-center gap-2" onClick={handleNavClick}>
